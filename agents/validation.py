...
def validation_agent(state: dict) -> dict:
    ...
    retry_count = state["control"].get("retry_count", 0)
    ...
    if not state["result"].get("query_result"):
        if retry_count < MAX_RETRIES:
            return {
                **state,
                "result": {
    ...
    columns = state["result"].get("column_names", [])
    rows = state["result"]["query_result"]
    ...
    prompt = VALIDATION_PROMPT.format(
        ...
        sql_query=state["result"]["sql_query"],
